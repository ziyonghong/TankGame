...
public class GameFrame extends Frame implements ActionListener {
	...
	public void framPaint(Graphics g) {
		...
		// 创建一个JPanel，添加显示信息，面板添加到窗体的北边
		JPanel northPane = new JPanel();
		northPane.setPreferredSize(new Dimension(0, 0));// 设置组件的大小
		Font f1 = g.getFont();
		g.setFont(new Font("TimesRoman", Font.BOLD, 20));
		g.drawString("区域内还有敌方坦克: ", 200, 80);
		g.setFont(new Font("TimesRoman", Font.ITALIC, 30));
		g.drawString("" + tanks.size(), 400, 80);
		g.setFont(new Font("TimesRoman", Font.BOLD, 20));
		g.drawString("剩余生命值: ", 500, 80);
		g.setFont(new Font("TimesRoman", Font.ITALIC, 30));
		g.drawString("" + homeTank.getLife(), 650, 80);
		g.setFont(f1);
